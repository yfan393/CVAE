...
class UKBBData(Dataset):
    ...
    def _load(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # ── ICA ───────────────────────────────────────────────────────────────
        # Raw shape: (x, y, z, 53). np.moveaxis gives a view, then
        # ascontiguousarray makes one C-contiguous copy.
        ...
        # Out-of-place multiplication avoids modifying shared numpy memory.
        ica_stacked = ica_stacked * (ica_stacked.abs() > ICA_THRESHOLD)

        # ── sMRI ──────────────────────────────────────────────────────────────
        # T1w sMRI is at 1 mm; resample to ICA grid before padding.
        # Linear interpolation is appropriate for continuous MRI intensities.
